      ...
      if (acceptsAnyObject || uaAccepted) {
        findings.push(finding({
          ...
          detail:
            ...
            + 'Nor does the forgery have to arrive before boot, or go through the handshake at all: the mark is one attribute on <html>, inAppPro() reads that attribute and nothing about who wrote it, and nothing re-checks it once the page is up — so document.documentElement.setAttribute(\'data-ib-pro\', \'\') typed into the DevTools console after the page has loaded grants exactly the same thing, with no bridge object, no user agent and no boot script involved. That is a second door, not a variant of the first, and it is why a stricter handshake could never close this: the handshake only decides who writes the attribute, and the attribute is what the gate reads. '
            + 'The honest answer is that both are ACCEPTABLE AS IT STANDS, and the reason is narrow: the mark decides wording and hides upgrade asks, and the three actions behind it are a CSV of cookies already listed on the page, a re-run of an audit that only reads this browser, and a second local photo. Self-granting buys a visitor nothing they could not already have, and costs the server nothing. '
          ...
          evidence:
            ...
            + 'both are set by the page\'s own environment, so any script with execution in our origin satisfies them. inAppPro() is a single hasAttribute(\'data-ib-pro\') read, so the attribute set by hand after load reads as a subscriber too — tests/pro-entitlement.test.ts runs the real inAppPro() and shouldGate() against exactly that document. Gated actions today: cookie-csv-export, browser-privacy-rerun, metadata-multi-file (lib/card-copy.ts GATE_COPY) — all client-side.',
