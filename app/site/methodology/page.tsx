...
export default function MethodologyPage() {
  ...
  return (
    <article className="max-w-3xl mx-auto prose prose-invert">
      ...
      <ul className="list-disc pl-6 text-[#B8B8D4] space-y-1">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm my-3">
          <thead className="text-xs uppercase tracking-wider text-[#B8B8D4]/60"><tr><th className="text-left py-2 pr-3">Finding</th><th className="text-left py-2 pr-3">Points</th><th className="text-left py-2">Cap</th></tr></thead>
          <tbody className="text-[#B8B8D4]">
            {[
              ...
              <tr key={a} className="border-t border-white/10"><td className="py-2 pr-3">{a}</td><td className="py-2 pr-3 font-mono">{b}</td><td className="py-2 font-mono">{c}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      <p className="text-[#B8B8D4]"><strong className="text-white">Grades:</strong> A ≥ 90 · B ≥ 78 · C ≥ 62 · D ≥ 45 · F below 45.</p>
      ...
      <p className="mt-8"><Link href="/site" className="text-sm text-[#B8B8D4] hover:text-white">← All report cards</Link></p>
    </article>
  );
}
