...
export async function generateStaticParams() {
  ...
  const items = getAllContentItems<CalculatorMeta>('calculators');
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return genMeta({
    title: `${nicheData?.name ?? niche} Privacy Calculators`,
    description: `Privacy risk calculators for ${nicheData?.name ?? niche}.`,
    path: `/calculators/${niche}`,
    type: 'website',
  });
}

export default async function CalculatorsByNiche({ params }: PageProps) {
  const { niche } = await params;
  const items = getAllContentItems<CalculatorMeta>('calculators').filter(i => i._niche === niche);
